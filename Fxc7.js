...
const _simi = JSON.parse(fs.readFileSync('./database/json/simi.json'))
...
const { help, bahasa, donasi, limitend, limitcount, bottt, listephoto, listtextpro } = require('./Fxc7/help')
...
prefix = "#"
name = "~ FIDSBOT"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMAN FIDSBOT😁"
rmenu = "HAI TEMAN FIDSBOT👋"
...
function addMetadata(packname, author) {
				if (!packname) packname = 'bott'; if (!author) author = ' Fids';
...
async function starts() {
	...
	frhan.on('chat-update', async (mek) => {
       ...
       if (messagesLink.includes("://chat.whatsapp.com/")){
		...
		return reply( "Tolong Jaga Ucapan")
		}
	}
}
const checkLimit = (sender) => {
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		case 'verify':
            ...
            break  
		        ...
		        frhan.sendMessage(from, buffer, image, {quoted: mek, caption: `*NAME* : ${groupName}\n*MEMBER* : ${groupMembers.length}\n*ADMIN* : ${groupAdmins.length}\nMODE WELCOME:${a} *${Welcome}*\nMODE SIMI:${a} *${Simi}*\nMODE ANIME:${a} *${ModeAnime}*\nMODE NSFW:${a} *${Nsfw}*\nMODE ANTI BADWORD:${a} *${BadWord}*\nMODE ANTI LINK:${a} *${AntiLink}*\nMODE ANTI VIRTEX:${a} *${AntiVirtex}*\n*DESK* : ${groupDesc}`})
                break
				case 'trendtwit':
					frhan.updatePresence(from, Presence.composing) 
                     if (!isUser) return reply(mess.only.userB)
		...
		break 
			...
			case 'bts':
				...
				frhan.sendMessage(from, buff, image, {quoted: mek, caption: 'BTS.....'})
					...
					await limitAdd(sender)
			case 'antivirtex':
			await costum(antivirtexx(), text, FarhanGans, `Buset Dahh`)
			break 
			case 'addstatus':
					if (!isOwner) return reply(mess.only.ownerB)
